"""Minimal SAGE-Reg prototype for distributional self-agreement in regression."""
...
class SelfAgreementTrainer:
    """Tiny trainer wrapper for SAGE-Reg paper prototypes.

    The trainer assumes:
    - ``supervised_loss_fn(model, x_labeled, y_labeled) -> Tensor``
    - ``predictive_batch_fn(model, x) -> PredictiveBatch``
    - labeled loaders yield ``(x, y, ...)`` or ``(x, y)``
    - unlabeled loaders yield ``x`` or ``(x, ...)``
    """
    ...
    def compute_consensus(self, predictive_views: Sequence[PredictiveBatch]) -> PredictiveBatch:
        return build_consensus_predictive_batch(
            predictive_views,
            n_support=self.objective.n_support,
            range_margin=self.objective.range_margin,
            gaussian_std_span=self.objective.gaussian_std_span,
            min_scale=self.objective.min_scale,
    ...
    def unsupervised_loss(
        ...
        weights = weight.detach() if self.objective.detach_weights else weight
        ...
        *,
        ...
        for epoch_idx in range(epochs):
            ...
            for labeled_batch, unlabeled_batch in zip(
                ...
                total_loss = supervised_loss + self.objective.agreement_weight * unsupervised_loss
                total_loss.backward()
                self.optimizer.step()
