...
_NEURIPS_HIGGS = REPO_ROOT / "data" / "neurips_inputs" / "FAIR_Universe_HiggsML_data.parquet"
_LEGACY_HIGGS = (
    REPO_ROOT
    / "docs/research/sage_reg_results/2026-04-09/higgs_public/extracted"
    / "FAIR_Universe_HiggsML_data.parquet"
)
HIGGS_PARQUET_DEFAULT = _NEURIPS_HIGGS if _NEURIPS_HIGGS.is_file() else _LEGACY_HIGGS
...
def main() -> None:
    ...
    p.add_argument(
        "--higgs-parquet",
        type=Path,
        default=HIGGS_PARQUET_DEFAULT,
        help=(
            "Default: data/neurips_inputs/FAIR_Universe_HiggsML_data.parquet if present, "
            "else legacy docs/research/.../extracted/ path (see docs/canfar_neurips_batch.md)."
        ),
    )
    p.add_argument(
        "--skip-higgs",
