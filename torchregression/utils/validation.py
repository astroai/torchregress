...
and ensure they meet the required constraints.
"""
    ...
    if isinstance(value, torch.Tensor):
        if allow_zero:
            if torch.any(value < 0):
                raise ValueError(f"{param_name} must be non-negative, got negative values")
        else:
            if torch.any(value <= 0):
                raise ValueError(f"{param_name} must be positive, got non-positive values")
    else:
        if allow_zero:
            if value < 0:
...
def validate_non_negative(value: Union[float, torch.Tensor], name: str = "value") -> Union[float, torch.Tensor]:
    ...
    if isinstance(value, torch.Tensor):
        if torch.any(value < 0):
            raise ValueError(f"{name} must be non-negative, got minimum value {value.min().item()}")
    else:
        if value < 0:
