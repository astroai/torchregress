...
class MixtureDensityLoss(DistributionLoss):
    ...
    def __init__(self, n_components: int, n_features: int, covariance_type: str = 'diagonal',
        ...
        if self.covariance_type not in ['diagonal', 'full']:
            raise ValueError(f"Unsupported covariance_type: {covariance_type}. Expected 'diagonal' or 'full'")
    ...
    def _extract_distribution_parameters(self, y_pred):
        ...
        if self.covariance_type == 'diagonal':
            ...
            for c in range(self.n_components):
                ...
                L = torch.zeros(*batch_shape, self.n_features, self.n_features, 
    ...
    def _log_prob_full(self, y_true, means, L_matrices):
        ...
        for c in range(self.n_components):
            ...
            try:
                ...
                z = torch.linalg.solve_triangular(L, comp_residuals.unsqueeze(-1), upper=False).squeeze(-1)
