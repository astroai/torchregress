...
from ..utils.labels import (
    create_bin_edges,
    calculate_bin_properties,
    create_gaussian_target_distribution,
    create_ordinal_encoding_matrix,
    convert_to_ordinal_targets,
    decode_bin_probabilities,
)
...
class BinnedRegressionLoss(DistributionLoss):
    ...
    def _get_target_distribution(
        self, target: torch.Tensor, uncertainty: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        ...
        if self.soft_targets:
            ...
            if sigmas.ndim == 1:
                ...
                target_probs = torch.zeros(
                    batch_size, self.n_bins, device=target.device, dtype=target.dtype
                )
        ...
        if self.soft_targets:
            ...
            if self.loss_type == "cross_entropy":
                ...
                loss = F.cross_entropy(
                    logits, target_indices, reduction="none", label_smoothing=self.label_smoothing
                )
...
class OrdinalRegressionLoss(BinnedRegressionLoss):
    ...
    def _extract_distribution_parameters(self, y_pred: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        probs = torch.zeros(batch_size, self.n_bins, device=y_pred.device, dtype=y_pred.dtype)
    ...
    def _calculate_nll(
        ...
        if self.loss_type == "kl_div":
            ...
            if self.wasserstein_p == 1:
                ...
                # Wp distance
                loss = torch.sum(torch.abs(target_cdf - pred_cdf) ** self.wasserstein_p, dim=1) ** (
                    1.0 / self.wasserstein_p
                )
...
class RegressionAsClassificationLoss(BinnedRegressionLoss):
    ...
    def _extract_distribution_parameters(self, y_pred: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.order_aware and y_pred.shape[1] == self.n_bins - 1:
            ...
            # Calculate class probabilities from binary probabilities
            probs = torch.zeros(
                y_pred.shape[0], self.n_bins, device=y_pred.device, dtype=y_pred.dtype
            )
    ...
    def forward(
        ...
        if self.order_aware and y_pred.shape[1] == self.n_bins - 1:
            ...
            raise ValueError(f"Expected y_pred with {self.n_bins} outputs, got {y_pred.shape[1]}")
        ...
        if self.order_aware and y_pred.shape[1] == self.n_bins - 1:
            ...
            if self.soft_targets:
                ...
                ordinal_targets = torch.zeros(
                    batch_size, self.n_bins - 1, device=target.device, dtype=torch.float32
                )
                for i in range(batch_size):
            ...
            if self.soft_targets:
                ...
                if self.loss_type == "cross_entropy":
                    ...
                    loss = F.cross_entropy(params["logits"], bin_indices.long(), reduction="none")
                elif self.loss_type == "focal":
                    # Focal loss for hard targets
                    log_probs = F.log_softmax(params["logits"], dim=1)
                    ...
                    target_probs = (
                        params["bin_probs"].gather(1, bin_indices.long().unsqueeze(1)).squeeze()
                    )

                    # Apply focal weighting
                    focal_weight = (1 - target_probs) ** self.focal_gamma
                    ce_loss = F.cross_entropy(
                        params["logits"], bin_indices.long(), reduction="none"
                    )
...
def create_classification_regression_loss(
    bins: Union[int, torch.Tensor] = 10, **kwargs
...
def create_noise_aware_regression_loss(
    bins: Union[int, torch.Tensor] = 20, method: str = "histogram", **kwargs
) -> BinnedRegressionLoss:
    """Create a regression loss designed for noisy labels."""
    return create_binned_regression_loss(
        method=method, bins=bins, noise_aware=True, adaptive_sigma=True, **kwargs
    )
...
def uncertainty_regression(
    bins: int = 20, min_value: Optional[float] = None, max_value: Optional[float] = None, **kwargs
) -> BinnedRegressionLoss:
    """
