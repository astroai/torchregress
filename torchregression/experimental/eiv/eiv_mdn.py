...
from .eiv_utils import prepare_sigma, compute_model_gradients, prepare_model_input_for_gradients, generate_perturbed_samples
...
class MDNEIVLoss(RegressionLoss):
    ...
    def forward(self, x_obs, y_true, y_pred, mask=None):
        ...
        if self.uncertainty_method == 'fixed':
            ...
            adjusted_sigmas = self._monte_carlo_uncertainty_adjustment(x_obs, sigmas, sigma_x, sigma_y)
        else:
    ...
    def _fixed_uncertainty_adjustment(self, sigmas, sigma_x, sigma_y):
        ...
        if sigma_y is not None:
            if sigma_y.ndim <= 1:  # diagonal case
                ...
                sigma_y_diag = torch.diagonal(sigma_y).unsqueeze(0).unsqueeze(0).expand_as(adjusted_sigmas)
        ...
        if sigma_y is not None:
            if sigma_y.ndim <= 1:  # diagonal case
                for k in range(self.num_components):
                    ...
                    adjusted_sigmas[:, k] = torch.sqrt(adjusted_sigmas[:, k]**2 + sigma_y_expanded**2)
            else:
    ...
    def _logsumexp(self, tensor, dim=-1):
        ...
        result = torch.log(torch.sum(torch.exp(tensor_stable), dim=dim) + self.eps) + max_val.squeeze(dim)
        return result
    ...
    def sample_from_mixture(self, logits, means, sigmas, n_samples=1):
        ...
        for i in range(batch_size):
            ...
            for j in range(n_samples):
                k = components[j]
                samples[i, j] = means[i, k] + sigmas[i, k] * torch.randn(self.n_features, device=device)
...
class MDNEIVModel(nn.Module):
    ...
    def sample(self, x: torch.Tensor, n_samples: int = 1) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            sigma = torch.exp(params['log_sigmas']).clamp(min=1e-4)  # [batch_size, num_components, output_size]
    ...
    def get_uncertainty(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            expected_comp_var = torch.sum(probs.unsqueeze(-1) * sigma**2, dim=1)  # [batch_size, output_size]
            ...
            var_comp_means = torch.sum(probs.unsqueeze(-1) * centered_mu**2, dim=1)  # [batch_size, output_size]
            ...
            comp_entropy = torch.sum(0.5 * torch.log(2 * torch.pi * torch.e * sigma**2), dim=2)  # [batch_size, num_components]
