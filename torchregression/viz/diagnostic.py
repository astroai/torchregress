...
def plot_reliability_diagram(
    ...
    calibration_metrics = expected_calibration_error(y_pred_quantiles, y_true, return_diagnostics=True)
...
def plot_residuals(
    ...
    if show_trend and len(y_pred) > 1:
        try:
            ...
            p = np.poly1d(z)
            ax.plot(np.sort(y_pred), p(np.sort(y_pred)), 
                    color=trend_color, linestyle='--', 
...
def plot_prediction_intervals(
    ...
    ax.fill_between(x, y_lower, y_upper, alpha=alpha, color=color_interval, label='Prediction Interval')
...
def plot_distribution_comparison(
    ...
    if len(pred_samples.shape) != 2:
        raise ValueError(f"predicted_samples should have shape [n_samples, batch_size], got {pred_samples.shape}")
    ...
    for i, idx in enumerate(indices):
        ...
        if len(valid_samples) < len(samples):
            print(f"Warning: {len(samples) - len(valid_samples)} non-finite values removed from samples")
        ...
        if plot_type in ['kde', 'both'] and has_kde and len(samples) >= 3:
            try:
                ...
                if len(x_ci) > 0:
                    y_ci = kde(x_ci)
                    ax.fill_between(x_ci, 0, y_ci, color=color_samples, alpha=0.2, 
                ...
                ax.hist(samples, bins=min(20, len(samples) // 5 + 1), 
                       alpha=0.3, color=color_samples, density=True)
        ...
        if np.isfinite(true_value):
            ...
            ax.text(true_value, 0.01, f"{true_value:.2f}", color=color_true,
        ...
        # Add CI boundaries
        ax.axvline(x=lower_ci, color='orange', linestyle='-.', linewidth=1, alpha=0.7, label='CI Bounds')
...
def plot_calibration_curve(
    ...
    if np.min(y_pred_probs) < 0 or np.max(y_pred_probs) > 1:
        print(f"Warning: Predicted probabilities outside [0, 1] range: min={np.min(y_pred_probs)}, max={np.max(y_pred_probs)}")
    ...
    if add_hist:
        ...
        for i in range(len(scaled_hist)):
            ax.bar(prob_pred[i], scaled_hist[i], width=(1/n_bins), 
                  bottom=-scaled_hist[i], align='center', alpha=hist_alpha, color=hist_color)
