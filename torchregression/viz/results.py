...
def plot_performance_comparison(
    ...
    if higher_is_better is None:
        ...
        for metric in metrics_to_include:
            ...
            is_higher_better = not any(term in metric.lower() 
                                      for term in ['error', 'loss', 'mae', 'mse', 'rmse', 'mape'])
    ...
    if plot_type == 'bar':
        return _plot_performance_bar(
            df, best_values, figsize, title, higher_is_better,
            colors, return_figure, ax
        )
    elif plot_type == 'radar':
        return _plot_performance_radar(
            df, best_values, figsize, title, higher_is_better,
            colors, return_figure
        )
...
def _plot_performance_bar(
    ...
    for i, model_name in enumerate(df.index):
        ...
        for j, v in enumerate(values):
            ...
            ax.text(indices[j] + i * bar_width, v * 1.01, value_str, 
    ...
    for i, metric in enumerate(df.columns):
        ...
        ax.annotate(direction, xy=(i, 0), xytext=(0, 10), 
...
def _plot_performance_radar(
    ...
    for metric in df.columns:
        ...
        if higher_is_better.get(metric, True):
            ...
            df_normalized[metric] = (df[metric] - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        else:
            # Lower is better: lower values should be further from center
            df_normalized[metric] = 1 - (df[metric] - min_val) / (max_val - min_val) if max_val > min_val else 0.5
    ...
    for i, (metric, values) in enumerate(metric_values.items()):
        ...
        for param_name, param_values in parameter_values.items():
            ...
            if plot_type == 'line':
                ...
                if highlight_best:
                    ax.scatter([sorted_params[best_idx]], [sorted_values[best_idx]], 
...
def plot_feature_importance(
    ...
    # Create bars
    if horizontal:
        bars = ax.barh(y_pos, importance_values, color=color, 
                      xerr=importance_errors, error_kw=dict(ecolor=error_color))
        ax.set_yticks(y_pos)
        ...
        bars = ax.bar(y_pos, importance_values, color=color, 
                     yerr=importance_errors, error_kw=dict(ecolor=error_color))
        ax.set_xticks(y_pos)
