...
def plot_learning_curves(
    ...
    for i, metric in enumerate(metrics_to_plot):
        ...
        # Plot training curve
        ax.plot(epochs, train_smoothed,
                label=f"Train {metric}", color=color_train, marker=None)
        ...
        if val_history is not None and metric in val_history:
            ...
            if not np.all(val_valid_idx):
                print(f"Warning: {np.sum(~val_valid_idx)} non-finite values removed from validation {metric}")
            ...
            ax.plot(val_epochs, val_smoothed,
                   label=f"Val {metric}", color=color_val, marker='o', markersize=4)
        ...
        if show_annotations and len(train_values) > 0:
            is_loss = any(term in metric.lower() for term in ["loss", "error", "mae", "mse", "rmse"])
...
def plot_validation_metrics(
    ...
    for i, (metric_name, values) in enumerate(metrics.items()):
        ...
        if highlight_best:
            ...
            ax.scatter([best_epoch], [best_value], color='red', s=100, 
                      marker='*', label=f'Best: {best_value:.4f} (epoch {best_epoch})')
...
def plot_early_stopping(
    ...
    ax.axvspan(waiting_start, waiting_end, alpha=0.2, color='red',
