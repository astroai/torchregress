...
def probability_integral_transform(
    ...
    if return_histogram:
        ...
        return {
            'pit_values': pit_values.cpu().numpy() if isinstance(y_true, np.ndarray) else pit_values,
            'histogram_counts': normalized_counts.cpu().numpy() if isinstance(y_true, np.ndarray) else normalized_counts,
            'bin_edges': bin_edges.cpu().numpy() if isinstance(y_true, np.ndarray) else bin_edges,
            'uniformity_chi2': torch.sum((normalized_counts - 1.0)**2).item()  # Chi-square statistic against uniformity
        }
