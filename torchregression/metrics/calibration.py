...
def marginal_calibration_error(
    ...
    for i in range(y_pred_samples.shape[0]):  # Loop through samples
        ...
        pred_cdf = torch.cumsum(pred_hist, dim=0) / max(1, len(y_pred_samples[i]))  # Avoid division by zero
...
def calibration_metrics_report(
    distribution_or_samples: Union[torch.distributions.Distribution, Dict[str, torch.Tensor], torch.Tensor],
    y_true: Union[torch.Tensor, np.ndarray],
