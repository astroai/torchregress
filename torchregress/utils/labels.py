...
def combine_dawid_skene(
    ...
    if init_confusion is None:
        ...
        confusion_matrices = (
            torch.eye(num_classes, device=device).unsqueeze(0).expand(num_annotators, -1, -1)
            + noise
        )
        ...
        if init_confusion.shape != (num_annotators, num_classes, num_classes):
            raise ValueError(
                f"Expected init_confusion shape ({num_annotators}, {num_classes}, {num_classes}), got {init_confusion.shape}"
            )
...
def combine_continuous_blue_with_scaling(
    ...
    if covariance_matrix is not None:
        ...
        if covariance_matrix.ndim != 2:
            raise ValueError(
                f"covariance_matrix must be a 2D tensor, got shape {covariance_matrix.shape}"
            )
        if covariance_matrix.shape != (num_estimators, num_estimators):
            raise ValueError(
                f"covariance_matrix shape {covariance_matrix.shape} must match (num_estimators, num_estimators) = ({num_estimators}, {num_estimators})"
            )
        ...
        jitter = (
            1e-6
            * torch.eye(num_estimators, device=device)
            * torch.max(torch.diag(covariance_matrix))
        )
        ...
        try:
            ...
            V_inv = torch.linalg.pinv(
                covariance_matrix + 1e-4 * torch.eye(num_estimators, device=device)
            )
        ...
        if variances.shape[0] != num_estimators:
            raise ValueError(
                f"variances shape {variances.shape} must match (num_estimators,) = ({num_estimators},)"
            )
