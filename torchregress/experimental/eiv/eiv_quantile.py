...
class MultiQuantileEIVLoss(BaseEIVLoss):
    ...
    def forward(self, x_obs, y_true, mask=None):
        ...
        # Forward pass for all samples
        with torch.no_grad():
            y_preds_flat = self.model(
                x_flat
            )  # Should output [n_samples * batch_size, n_features_y, n_quantiles]

            # Reshape predictions
            n_features_y = y_true.shape[1]
            y_preds = y_preds_flat.reshape(
                self.n_samples, batch_size, n_features_y, self.n_quantiles
            )

            # Apply mask if provided
            if mask is not None:
                mask_expanded = (
                    mask.unsqueeze(0).unsqueeze(-1).expand(self.n_samples, -1, -1, self.n_quantiles)
                )
                y_preds = torch.where(mask_expanded, y_preds, torch.zeros_like(y_preds))
        ...
        y_expanded = y_true.unsqueeze(-1).expand(
            -1, -1, self.n_quantiles
        )  # [batch_size, n_features_y, n_quantiles]
