...
def train_model(model, loss_fn, train_loader, epochs=50, lr=0.001, device="cpu", use_augmentation=False):
    ...
    for epoch in range(epochs):
        for x_batch, x_err_batch, y_batch, y_err_batch in train_loader:
            ...
            if isinstance(loss_fn, FunctionalEIVLoss):
                loss = loss_fn(y_pred, y_batch, sigma_x=x_err_batch.to(device), sigma_y=y_err_batch.to(device))
            else:
...
def main():
    ...
    losses_to_compare = {
        "MSE": {"loss": create_loss_from_config({"type": "mse"}), "model": PhotoZMLP(len(feature_cols))},
        "GaussianNLL": {"loss": create_loss_from_config({"type": "gaussian_nll"}), "model": PhotoZMLP(len(feature_cols), 2)},
        "FunctionalEIV": {"loss": FunctionalEIVLoss(model=PhotoZMLP(len(feature_cols))), "model": PhotoZMLP(len(feature_cols))},
        "TukeyBiweight": {"loss": TukeyBiweightLoss(), "model": PhotoZMLP(len(feature_cols))},
        "Quantile (0.5)": {"loss": QuantileLoss(quantile=0.5), "model": PhotoZMLP(len(feature_cols))},
        "Ensemble (MSE)": {"loss": create_loss_from_config({"type": "mse"}), "model": DeepEnsemble(base_model=PhotoZMLP(len(feature_cols)), ensemble_size=5)},
    }

    results = {}
    for name, config in losses_to_compare.items():
        print(f"\n--- Training {name} ---")
        model = config["model"]
        loss_fn = config["loss"]
        ...
        if name == "GaussianNLL":
            ...
            # The output of the ensemble is a list of tensors, so we stack them
            y_pred_ensemble = torch.stack(evaluate_model(model, test_loader, device)[0], dim=0)
            samples = y_pred_ensemble.permute(1, 0, 2)
            y_pred = y_pred_ensemble.mean(0)
    ...
    for i, (name, result) in enumerate(results.items()):
        ...
        if name == "Ensemble (MSE)":
            y_pred = torch.stack(y_pred, dim=0).mean(0)
