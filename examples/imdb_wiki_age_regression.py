...
def download_and_extract_dataset():
    ...
    if os.path.exists(os.path.join(DATA_DIR, 'wiki_crop')) and \
       len(os.listdir(os.path.join(DATA_DIR, 'wiki_crop'))) > 0:
        ...
        with requests.get(IMDB_WIKI_URL, stream=True) as r:
            ...
            with open(zip_path, 'wb') as f:
                ...
                for chunk in r.iter_content(chunk_size=8192):
                    ...
                    print(f"\r[{'=' * done}{' ' * (50-done)}] {downloaded/1024/1024:.1f}/{total_size/1024/1024:.1f} MB", 
                          end='', flush=True)
        print("\nDownload completed.")
...
def create_metadata():
    ...
    filtered_df = df[(df['age'] >= 0) & (df['age'] <= 100) & 
                      (df['face_score'] > 1.0) & 
...
def main():
    ...
    # Add identity line
    min_val = min([np.min(results[loss_name]['metrics']['targets']) for loss_name in loss_functions])
    max_val = max([np.max(results[loss_name]['metrics']['targets']) for loss_name in loss_functions])
    ...
    for loss_name, result in results.items():
        metrics = result['metrics']
        print(f"{loss_name:<15} {metrics['mae']:<10.2f} {metrics['mse']:<10.2f} {metrics['rmse']:<10.2f}")
