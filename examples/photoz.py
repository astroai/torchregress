...
from torchregression.losses.eiv import (
    EIVRegressionLoss, 
    OrthogonalEIVLoss,
    WeightedEIVLoss
)
...
def create_simulated_sdss_data(n_galaxies=5000):
    ...
    df = pd.DataFrame({
        ...
        'r': r, 'r_err': r_err,
        'i': i, 'i_err': i_err,
        'z_mag': z, 'z_mag_err': z_err,
        'spec_z': z_spec, 'spec_z_err': z_err
    })
...
class SDSSDataset(Dataset):
    ...
    def __init__(self, data, feature_cols, error_cols, target_col='spec_z', 
        ...
        self.feature_errors = data[error_cols].values / np.sqrt(self.feature_scaler.var_)[:, np.newaxis].T
...
def plot_results(results, loss_names):
    ...
    for i, name in enumerate(loss_names):
        ...
        idx = np.random.choice(len(metrics['predictions']), 
                              size=min(500, len(metrics['predictions'])), 
                              replace=False)
        
        plt.scatter(metrics['targets'][idx, 0], 
...
def main():
    ...
    for loss_name, result in results.items():
        ...
        print(f"{loss_name:<15} {metrics['mae']:<10.4f} {metrics['rmse']:<10.4f} "
