...
def download_sdss_data(force_download=False, sample_size=10000):
    ...
    try:
        response = requests.post(
            SDSS_API_URL, data={"cmd": sql_query, "format": "csv"}, timeout=120
        )
...
class PhotoZExperiment:
    ...
    def setup_data(self):
        ...
        # Split dataframe indices (deterministic)
        ...
        self.train_ds = SDSSDataset(train_df, self.feature_cols, self.error_cols)
        self.scaler = self.train_ds.feature_scaler
        self.val_ds = SDSSDataset(
            val_df, self.feature_cols, self.error_cols, feature_scaler=self.scaler
        )
        self.test_ds = SDSSDataset(
            test_df, self.feature_cols, self.error_cols, feature_scaler=self.scaler
        )
    ...
    def evaluate(self, model, loss_fn, model_type, stress_type=None):
        ...
        try:
            ...
            q_dict = {0.16: preds - stds, 0.50: preds, 0.84: preds + stds}
            ece_result = ExpectedCalibrationError()(q_dict, targets)
            ece = ece_result["mean_absolute_calibration_error"]
        except Exception:
            ece = float("nan")
        ...
        for name, (model, loss_fn, mtype) in trained_models.items():
            for scen in scenarios:
                ...
                metrics = self.evaluate(
                    model, loss_fn, mtype, stress_type=scen if scen != "baseline" else None
                )
