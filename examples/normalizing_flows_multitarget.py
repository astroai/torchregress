"""
Multi-Target Distributional Regression with Normalizing Flows

This example demonstrates how to use normalizing flows for multi-target regression
with complex, potentially multimodal distributions. Normalizing flows can model:

1. Multiple correlated targets simultaneously
2. Complex non-Gaussian distributions (multimodal, skewed, etc.)
3. Input-dependent distributions (aleatoric uncertainty)
4. Epistemic uncertainty (when combined with ensembles)

Use cases:
- Trajectory prediction (x, y coordinates over time)
- Multi-output scientific measurements
