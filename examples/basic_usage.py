...
def train_model(model, loss_fn, x_train, y_train, epochs=200, lr=0.01, 
                uncertainty_model=False, verbose=True):
...
def main():
    ...
    gnll_losses = train_model(
        model_uncertainty, gaussian_nll, x_train, y_train, 
        uncertainty_model=True, verbose=False
    )
    ...
    plt.fill_between(
        ...
        alpha=0.3, color='r', label="95% prediction interval"
    )
