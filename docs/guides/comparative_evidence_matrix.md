...
| Causal inference regression (DR ATE/CATE) | `Strong` | `examples/causal_dr_uplift_comparison.py` | fixed seed, shared synthetic scenarios, matched nuisance models | ATE absolute error, CI contains true ATE, CI width, overlap rate, effective sample size, runtime | `dr_ate`, `dr_cate`, `naive difference-in-means` | Needs real treatment-effect benchmarks beyond synthetic scenarios. |
|  |  |  |  |  |  | _Note_: Current evidence includes synthetic uplift and astronomy-style selection-bias proxies with cross-fitted DR estimators and overlap diagnostics. |
