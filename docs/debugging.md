...
| **Poor Calibration (ECE high)** | Model is overconfident (variance too low) | `ExpectedCalibrationError`, `prediction_std` distribution | 1. Use `temperature_scaling`.<br>2. Add regularization (e.g. weight decay).<br>3. Check if targets have noise unseen by model. |
...
Losses involving logarithms (like `GaussianNLLLoss`, `Dirichlet`) are prone to numerical instability if the predicted parameters (like variance) approach zero.
...
**Fix:** Use `WeightedLossWrapper` or built-in weighted losses which handle reduction correctly.
