...
Start with a simple model and standard [`WeightedMSELoss`](../api/losses.md).
...
Switch to a heteroscedastic loss like [`GaussianNLLLoss`](../api/losses.md).
...
- **Check**: Compare [`WeightedHuberLoss`](../api/losses.md) vs [`WeightedMSELoss`](../api/losses.md) on a held-out test set with known outliers.
...
### Target Scaling
For many losses (like [`GaussianNLLLoss`](../api/losses.md)), it is often helpful to scale the targets $y$ to have zero mean and unit variance. This makes the initial "guess" of the model ($\mu \approx 0, \sigma \approx 1$) reasonable and improves convergence.
