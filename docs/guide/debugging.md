...
| **Poor Calibration (ECE high)** | Model is overconfident (variance too low) | [`ExpectedCalibrationError`](../api/metrics.md#expectedcalibrationerror), `prediction_std` distribution | 1. Use `temperature_scaling`.<br>2. Add regularization (e.g. weight decay).<br>3. Check if targets have noise unseen by model. |
