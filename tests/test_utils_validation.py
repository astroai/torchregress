...
def test_validate_positive():
    ...
    with pytest.raises(ValueError, match=r"weights must be positive, got tensor with minimum value 0.0"):
        validate_positive(t_zero, "weights", allow_zero=False)

    t_neg = torch.tensor([-1.0, 1.0])
    with pytest.raises(ValueError, match=r"weights must be non-negative, got tensor with minimum value -1.0"):
...
def test_validate_range():
    ...
    with pytest.raises(ValueError, match=r"probabilities must be between 0.0 and 1.0, got tensor with values outside range \[-0.1.*, 0.5\]"):
...
def test_validate_integer():
    ...
    with pytest.raises(ValueError, match="tensor must contain only integer values, got tensor with non-integer values"):
...
def test_validate_same_device():
    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            with pytest.raises(ValueError, match="Device mismatch: tensor_0 is on cpu, but tensor_1 is on mps:0"):
                validate_same_device([a, c])
        except Exception:
            pass
...
def test_check_tensor():
    ...
    with pytest.raises(ValueError, match="tensor contains 10 elements, which exceeds the maximum allowed limit of 5."):
...
def test_validate_same_device_mock():
    ...
    with pytest.raises(ValueError, match="Device mismatch: tensor_0 is on cpu, but tensor_1 is on mock_device:0"):
