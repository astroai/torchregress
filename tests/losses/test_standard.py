...
from torchregression.losses.standard import (
    MSELoss, BCELoss, MaskedMSELoss, MaskedL1Loss, MaskedBCELoss,
    MaskedBCEWithLogitsLoss, MaskedCrossEntropyLoss, MaskedKLDivLoss,
...
class TestMSELoss:
    ...
    def test_with_mask_and_weights(self):
        ...
        expected = torch.sum(weights_masked * (y_pred_masked - target_masked)**2) / torch.sum(weights_masked)
        assert torch.isclose(loss, expected)
...
class TestBCELoss:
    ...
    def test_with_mask(self):
        ...
        expected = -torch.mean(target_masked * torch.log(y_pred_masked) + (1 - target_masked) * torch.log(1 - y_pred_masked))
        assert torch.isclose(loss, expected)
    ...
    def test_with_pos_weight(self):
        ...
        expected = -torch.mean(pos_weight * target * torch.log(y_pred) + (1 - target) * torch.log(1 - y_pred))
        assert torch.isclose(loss, expected)
...
def test_gradient_preservation_all_losses():
    ...
    for loss_fn in losses:
        ...
        if loss_fn.__class__.__name__ in ['MaskedCrossEntropyLoss', 'MaskedNLLLoss', 'MaskedKLDivLoss']:
            continue
