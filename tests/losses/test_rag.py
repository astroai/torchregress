...
class TestBinnedRegressionLoss:
    ...
    def test_handle_out_of_range(self):
        ...
        loss_fn = BinnedRegressionLoss(bins=5, min_value=0.0, max_value=1.0, extrapolate_beyond_bins=True)
        ...
        loss_fn = BinnedRegressionLoss(bins=5, min_value=0.0, max_value=1.0, extrapolate_beyond_bins=False)
        target = torch.tensor([[-0.5], [0.2], [1.5]])
        result = loss_fn._handle_out_of_range(target)
...
class TestStandardClassificationRegressionLoss:
    ...
    def test_forward_soft_targets(self):
        ...
        logits = torch.tensor([
            [1.0, -1.0, 0.0],  # Predicts first bin
            [-1.0, 2.0, -1.0]  # Predicts second bin
        ])
    ...
    def test_different_loss_types_integration(self):
        ...
        # Cross entropy
        ce_loss = StandardClassificationRegressionLoss(
            bins=3, loss_type='cross_entropy'
        )
        ce_result = ce_loss(logits, targets)
        
        # KL divergence
        kl_loss = StandardClassificationRegressionLoss(
            bins=3, loss_type='kl_div'
        )
    ...
    def test_out_of_range_targets(self):
        ...
        loss_fn = StandardClassificationRegressionLoss(
            bins=3, min_value=0.0, max_value=1.0
        )
...
class TestOrdinalRegressionLoss:
    ...
    def test_ordinal_target_conversion(self):
        ...
        params = {
            'binary_logits': torch.randn(2, 3)  # batch_size x (n_bins-1)
        }
    ...
    def test_calculate_nll_different_loss_types(self):
        ...
        loss_fn = OrdinalRegressionLoss(
            bins=4, min_value=0.0, max_value=1.0
        )
    ...
    def test_forward_soft_targets(self):
        ...
        # Binary logits for 4 bins (3 thresholds)
        binary_logits = torch.tensor([
            [2.0, 1.0, -2.0],   # Predicts bin 2
            [-2.0, -1.0, -2.0]  # Predicts bin 0
        ])
...
class TestHistogramRegressionLoss:
    ...
    def test_calculate_nll_different_loss_types(self):
        ...
        pred_probs = torch.tensor([
            [0.7, 0.2, 0.1],  # Predicts mostly bin 0
            [0.1, 0.8, 0.1]   # Predicts mostly bin 1
        ])
    ...
    def test_forward_soft_targets(self):
        ...
        # Test with 'none' reduction
        loss_fn_none = HistogramRegressionLoss(
            bins=3, reduction='none', soft_targets=True
        )
...
class TestRegressionAsClassificationLoss:
    ...
    def test_init_and_params(self):
        ...
        loss_fn_no_ordinal = RegressionAsClassificationLoss(
            bins=5,
            order_aware=False
        )
    ...
    def test_forward_ordinal_mode_hard_targets(self):
        ...
        # Ordinal mode binary logits (n_bins-1 outputs)
        binary_logits = torch.tensor([
            [1.0, -1.0],  # Predicts bin 1
            [-1.0, 1.0]   # Predicts bin 0
        ])
    ...
    def test_uncertainty_handling(self):
        ...
        loss_fn = RegressionAsClassificationLoss(
            ...
            min_value=0.0, 
            max_value=1.0,
            noise_aware=True  # Enable uncertainty handling
        )
    ...
    def test_factory_function(self):
        ...
        loss_fn_robust = regression_as_classification(
            bins=5,
        ...
        loss_fn_small_bins = regression_as_classification(
            bins=8,
            auto_adapt=True
        )
        loss_fn_large_bins = regression_as_classification(
            bins=25,
            auto_adapt=True
        )
    ...
    def test_uncertainty_regression_factory(self):
        ...
        loss_fn = uncertainty_regression(
            bins=20,
            min_value=-1.0,
            max_value=1.0
        )
