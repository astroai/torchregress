...
class TestEIVLossNumericalStability:
    ...
    def test_multi_eiv_gradient_flow(self):
        ...
        assert gradcheck(loss_fn, (x_obs, y_obs, coefficients, bias, x_var, y_var), eps=1e-6, atol=1e-4)
    ...
    def test_nan_inf_handling(self):
        ...
        x_obs = torch.tensor([[1.0, 2.0, 3.0], [float('nan'), 5.0, 6.0], [7.0, 8.0, 9.0]], requires_grad=True)
        y_obs = torch.tensor([[1.0, 2.0], [3.0, 4.0], [float('inf'), 6.0]], requires_grad=True)
        x_var = torch.tensor([[0.1, 0.1, 0.1], [0.2, 0.2, 0.2], [0.3, 0.3, 0.3]], requires_grad=True)
