...
class TestGaussianLosses(unittest.TestCase):
    def setUp(self):
        ...
        self.simple_y = (
            torch.ones(1, 1, device=self.device) * 2
        )  # Prediction is 2, ground truth is 1
    ...
    def test_weighted_mse_loss(self):
        ...
        loss_weighted_masked = loss_fn(
            self.x, self.x_reconstructed, mask=self.mask, weights=weights
        )
    ...
    def test_diagonal_gaussian_nll(self):
        ...
        fixed_loss_fn = DiagonalGaussianNLL(learnable_variance=False, fixed_variance=0.5).to(
            self.device
        )
    ...
    def test_gaussian_nll_with_covariance(self):
        ...
        adj_loss_fn = GaussianNLLWithCovariance(
    ...
    def test_inf(self):
        ...
        self.assertFalse(
            torch.isinf(loss).any(), "Loss should not be inf when inf values are masked"
        )
        ...
        if self.mask[0, 0]:
            self.assertTrue(
                torch.isinf(loss).any(), "Loss should be inf with inf reconstructed values"
            )
        ...
        for loss_fn in loss_fns:
            with self.subTest(loss_fn=type(loss_fn).__name__):
                ...
                for reduction in ["none", "mean", "sum"]:
                    ...
                    if reduction == "none":
                        self.assertEqual(
                            loss.dim(), 1, f"Reduction 'none' should return per-sample losses"
                        )
                        self.assertEqual(
                            loss.shape[0], self.batch_size, f"Should have {self.batch_size} samples"
                        )
                    else:
                        self.assertEqual(
                            loss.dim(), 0, f"Reduction '{reduction}' should return a scalar"
                        )
...
class TestGaussianLossesNumericalStability:
    ...
    def test_extreme_values(self):
        ...
        y_true_small = torch.tensor(
            [1e-10 + 1e-12, 1e-15 + 1e-17, 1e-20 + 1e-22], dtype=torch.float
        )
