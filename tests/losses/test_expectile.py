...
from torchregression.losses.expectile import (
    ExpectileLoss, MultiExpectileLoss, ExpectileCrossover, AsymmetricLeastSquaresLoss
)
...
class TestExpectileLoss(unittest.TestCase):
    ...
    def test_multi_expectile_separate_predictions(self):
        ...
        y_pred_list = [
            torch.randn(batch_size, n_features, device=self.device)
            for _ in range(len(expectiles))
        ]
    ...
    def test_expectile_crossover_basics(self):
        ...
        self.assertLess(loss_good.item(), loss_bad.item(),
                        "Loss with crossover violations should be higher")
    ...
    def test_multi_expectile_nan_inf(self):
        ...
        self.assertFalse(torch.isnan(loss_target_nan).any(), "Loss should handle masked NaNs in targets")
    ...
    def test_multi_expectile_gradient(self):
        ...
        y_pred = torch.zeros(batch_size, num_expectiles, n_features, 
