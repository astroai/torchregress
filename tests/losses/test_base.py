...
from torchregression.losses.base import (
    BaseLoss, MaskedLoss, RegressionLoss, DistributionLoss, TorchLossWrapper
)
...
class TestGradientFlow:
    def test_gradient_flow_basic(self):
        ...
        expected_grad = torch.tensor([1.0, 0.0, -1.0]) / 3.0  # divided by 3 because of mean reduction
    ...
    def test_gradient_flow_distribution(self):
        ...
        y_pred = torch.tensor([
            [0.0, 0.0],  # mean=0, log_var=0
            [1.0, 0.0]   # mean=1, log_var=0
        ], requires_grad=True)
