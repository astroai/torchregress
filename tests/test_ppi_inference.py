...
def test_ppi_mean_ci_shape_mismatch() -> None:
    _, _, y_l, pred_l, pred_u = _synthetic()
    with pytest.raises(
        ValueError, match="y_labeled and pred_labeled must have the same number of samples"
    ):
...
def test_ppi_mean_ci_too_few_samples() -> None:
    _, _, y_l, pred_l, pred_u = _synthetic()
    with pytest.raises(
        ValueError, match="ppi_mean_ci requires at least 2 labeled and 2 unlabeled samples"
    ):
...
def test_ppi_quantile_ci_too_few_samples() -> None:
    _, _, y_l, pred_l, pred_u = _synthetic()
    with pytest.raises(
        ValueError, match="ppi_quantile_ci requires at least 2 labeled and 2 unlabeled samples"
    ):
...
def test_ppi_ols_ci_shape_mismatch() -> None:
    x_l, x_u, y_l, pred_l, pred_u = _synthetic()
    with pytest.raises(
        ValueError, match="x_labeled, y_labeled, and pred_labeled must align on sample dimension"
    ):
        ppi_ols_ci(x_l[:10], y_l[:15], x_u, pred_l[:10], pred_u)
    with pytest.raises(
        ValueError, match="x_unlabeled and pred_unlabeled must align on sample dimension"
    ):
