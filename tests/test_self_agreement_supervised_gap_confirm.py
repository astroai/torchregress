...
def test_self_agreement_supervised_gap_confirm_smoke(tmp_path: Path) -> None:
    mod = _load_module("self_agreement_supervised_gap_confirm")
    ...
    payload = mod.main(
        mod.SupervisedGapConfirmConfig(
    ...
    assert (out_dir / "selected_configs.json").exists()
    assert (out_dir / "year_confirm_summary.json").exists()
    assert (out_dir / "higgs_confirm_summary.json").exists()

    summary = json.loads((out_dir / "selected_configs.json").read_text(encoding="utf-8"))
    assert len(summary["runs"]) == 2
    assert summary["tuning_config"]["year_teacher_epochs"] == 2
