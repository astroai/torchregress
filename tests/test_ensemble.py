...
class TestBaseEnsembleModel:
    ...
    def test_initialization(self):
        ...
        for i in range(len(model.models)):
            for j in range(i+1, len(model.models)):
                ...
                for (p1, p2) in zip(model.models[i].parameters(), 
                                    model.models[j].parameters()):
                    assert p1 is not p2
    ...
    def test_forward(self):
        ...
        assert all(pred.shape == (5, 1) for pred in predictions)  # Each prediction should have shape [batch_size, output_size]
...
class TestUtilityFunctions:
    ...
    def test_run_ensemble_model(self):
        ...
        inputs_list = [torch.randn(10, 5) for _ in range(5)]  # 5 samples, each [batch_size, features]
