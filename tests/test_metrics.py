...
class TestCalibrationMetrics:
    ...
    def test_expected_calibration_error(self):
        ...
        result_diag = expected_calibration_error(
            self.y_pred_quantiles, self.y_true, return_diagnostics=True
        )
    ...
    def test_marginal_calibration_error(self):
        ...
        # Test with diagnostics
        result_diag = marginal_calibration_error(
            self.y_pred_samples, self.y_true, return_diagnostics=True
        )
...
class TestDistributionMetrics:
    ...
    def test_continuous_ranked_probability_score(self):
        ...
        crps_none = continuous_ranked_probability_score(
            self.y_pred_quantiles, self.y_true, reduction="none"
        )
        assert isinstance(crps_none, torch.Tensor)
        assert crps_none.shape == self.y_true.shape

        crps_sum = continuous_ranked_probability_score(
            self.y_pred_quantiles, self.y_true, reduction="sum"
        )
    ...
    def test_distribution_metrics_report(self):
        ...
        result = distribution_metrics_report(
            None, self.y_true, self.y_pred_quantiles, self.y_pred_samples
        )
        assert "crps" in result

        # Test with multi-dimensional data and samples
        result = distribution_metrics_report(
            None, self.y_true_multi, samples=self.y_pred_samples_multi
        )
...
class TestOODMetrics:
    ...
    def test_ood_metrics_report(self):
        ...
        result_partial = ood_metrics_report(model_output=self.model_output, x_test=self.x_test)
...
class TestIntervalMetrics:
    ...
    def test_prediction_interval_coverage(self):
        ...
        # Test with diagnostics
        result = prediction_interval_coverage_probability(
            self.lower, self.upper, self.y_true, return_diagnostics=True
        )
        assert isinstance(result, dict)
