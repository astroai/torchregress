...
def get_all_loss_classes() -> List[Type[Loss]]:
    ...
    for _, module_name, _ in pkgutil.iter_modules(losses_pkg.__path__):
        ...
        for name, obj in module.__dict__.items():
            if isinstance(obj, type) and issubclass(obj, Loss) and obj != Loss and obj != ReductionLoss:
...
def test_parameter_ordering():
    ...
    for loss_class in get_all_loss_classes():
        ...
        assert params[0] == 'y_pred', f"{loss_class.__name__}.forward() should have y_pred as first parameter, got {params[0]}"


def test_base_class_inheritance():
    """Test that all loss classes inherit from the base Loss class."""
    for loss_class in get_all_loss_classes():
        assert issubclass(loss_class, Loss), f"{loss_class.__name__} should inherit from Loss base class"
    ...
    for loss_class in get_all_loss_classes():
        ...
        for reduction in reduction_values:
            try:
                ...
                if reduction == 'none':
                    assert len(result.shape) >= 1, f"{loss_class.__name__} with reduction='none' should return non-scalar tensor"
                elif reduction in ('mean', 'sum'):
                    assert result.ndim == 0, f"{loss_class.__name__} with reduction='{reduction}' should return scalar"
                
            except Exception as e:
                pytest.fail(f"Reduction test failed for {loss_class.__name__} with {reduction}: {str(e)}")
...
def test_consistent_init_parameters():
    ...
    for loss_class in get_all_loss_classes():
        ...
        for param in common_params:
            assert param in param_names, f"{loss_class.__name__}.__init__() should have parameter named '{param}'"
